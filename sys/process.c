...
void create_task(task_struct *task, uint64_t main, uint64_t flags, uint64_t pagedir) {
	...
	task->regs.ebx = 0;
    	...
    	task->regs.esp = (uint64_t) main;// (uint64_t)(0xFFFFFFFF80000000+ allocate_page()) + 0x1000; // Not implemented here
    	task->next = 0;
}
...
//	__asm__ volatile("movq %0,%%rip;"::"r"(curr_task->regs.esp):"memory");
	...
	__asm__ volatile("movq %%rsp, %0":"=g"(curr_task->regs.esp)::);
